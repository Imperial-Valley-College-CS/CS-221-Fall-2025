...
public class App extends Application
{
   ...
   private void drawInvader( Invader vader )
   {
      ...
      int x = vader.getPosition().getX();
      int y = vader.getPosition().getY();
      helperDrawer( body, x, y );
   }   
   
   private void helperDrawer( boolean[][] body, int x, int y )
   {
      int savedX = x;
   ...
   private void shoot()
   {
      int y = Constants.HEIGHT - Constants.PLAYER_HEIGHT;
      int x = player.getPosition().getX() + Constants.PLAYER_WIDTH/2 + Constants.SCALE;
